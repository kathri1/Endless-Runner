...
SPIELER_DUCK = (60, 160, 220)   # NEU: Farbe beim Ducken
HINDERNIS = (255,  80,  80)
FLUG_F    = (255, 180,  50)     # NEU: Farbe fliegendes Hindernis
WOLKE_F   = (60,  60,  90)
WEISS     = (220, 220, 255)
GELB      = (255, 220,  60)
...
pygame.display.set_caption("Endless Runner v2 – Doppelsprung & Ducken")
...
class Spieler:
    BREITE       = 40
    HOEHE_NORMAL = 50
    HOEHE_DUCK   = 25   # NEU: halbe Höhe beim Ducken
    ...
    def reset(self):
        ...
        self.vy             = 0.0
        self.am_boden       = True
        self.sprung_zaehler = 0     # NEU: 0/1/2 – Doppelsprung
        self.duckt          = False # NEU: True wenn Pfeil-unten gedrückt
    ...
    def springe(self):
        # NEU: bis zu 2 Sprünge erlaubt
        if self.sprung_zaehler < 2:
            self.vy = SPRUNG_KRAFT
            self.am_boden = False
            self.sprung_zaehler += 1

    def update(self, duckt):
        self.duckt = duckt and self.am_boden  # Ducken nur am Boden
        ...
        if self.y >= boden:
            ...
            self.vy = 0
            self.am_boden = True
            self.sprung_zaehler = 0  # NEU: Reset bei Landung

    def rect(self):
        y = BODEN_Y - self.HOEHE if self.duckt else int(self.y)
        return pygame.Rect(int(self.x), y, self.BREITE, self.HOEHE)
    ...
    def zeichne(self, surf):
        ...
        # NEU: Gelber Punkt = noch ein Sprung verfügbar
...
class Hindernis:
    ...
    def zeichne(self, surf):
        ...
        # Spitze
        punkte = [(r.left, r.top), (r.right, r.top),
                  (r.centerx, r.top - 12)]
        pygame.draw.polygon(surf, (200, 50, 50), punkte)


# NEU: Fliegendes Hindernis – schwebt in mittlerer Höhe, muss unterduckt werden
class FliegendesHindernis:
    ...
    def __init__(self, geschwindigkeit):
        self.x     = float(BREITE + 20)
        # Schwebt ca. in Duckhöhe – zu hoch zum Drüberspringen, muss geduckt werden
        self.y     = BODEN_Y - 55
        self.speed = geschwindigkeit
    ...
    def zeichne(self, surf):
        r = self.rect()
        # Körper (Trapez-Form wie ein Vogel/UFO)
...
def spiel():
    ...
    geschwindigkeit = 5.0
    boden_offset  = 0.0
    naechstes_hindernis = random.randint(60, 120)  # Frames bis zum nächsten
    frame         = 0
    laufend       = True
    game_over     = False
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key in (pygame.K_SPACE, pygame.K_UP):
                    if game_over:
                        return spiel()   # Neustart
                    spieler.springe()

        if not game_over:
            frame += 1
            punkte = frame // 10
            geschwindigkeit = 5.0 + punkte * 0.01  # Wird schneller

            # ── Spieler Update (NEU: Duck-Taste übergeben) ──
            tasten = pygame.key.get_pressed()
            spieler.update(tasten[pygame.K_DOWN])
            ...
            # ── Hindernisse spawnen (NEU: 30% Chance auf fliegendes Hindernis) ──
            ...
            # ── Kollision ──
            sp_rect = spieler.rect().inflate(-8, -8)  # etwas kleiner = fairer
            for h in hindernisse:
